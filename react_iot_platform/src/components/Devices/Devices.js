    ...
    return (
      <div className="container-fluid device-table">
        {this.props.devices.length ? (
                  ...
                  this.props.devices.map((device, index) => {
                    return (
                      <tr>
                        ...
                        <td>
                          <NavLink to={`/updatedevice/` + index}>
                            <button
                              type="button"
                              id="edit"
                              name="edit"
                              className="device-btn btn btn-sm btn-primary mx-1 my-2"
                            >
                              <MdEdit />
                            </button>
                          </NavLink>
                          <button
                            ...
                            id="delete"
                            name="delete"
                            className="device-btn btn btn-sm btn-danger mx-1 my-2"
                            onClick={() => this.handleDelete(device.id)}
                          >
                            <MdDelete />
                          </button>
                          {device.is_root ? null : (
                            <NavLink to={`/device/` + index}>
                              <button
                                type="button"
                                id="edit"
                                name="edit"
                                className="device-btn btn btn-sm btn-dark mx-1 my-2"
                                onClick={() => {
                                  this.props.fetchData(device.id);
                                }}
                              >
                                <MdVisibility />
                              </button>
          ...
          <div className="alert alert-info">No devices to display...!</div>
        )}
      </div>
    );
  }
